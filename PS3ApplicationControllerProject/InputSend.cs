...
namespace PS3ApplicationControllerProject
{
    public class InputSend
    {
        ...
        public void SendKeyAsInput(System.Windows.Forms.Keys key, bool isKeyDown)
        {
            INPUT structure = new INPUT();
            structure.type = 1;
            structure.U.ki.wVk = (VirtualKeyShort)(short)key;

            uint scancode = MapVirtualKey((uint)key, 0);
            structure.U.ki.wScan = (ScanCodeShort)scancode;

            if (isKeyDown)
            {
                structure.U.ki.dwFlags = KEYEVENTF.SCANCODE;
            }
            else
            {
                structure.U.ki.dwFlags = KEYEVENTF.KEYUP;
            }
